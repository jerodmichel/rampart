...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  9 21:25:25 2026
...
class FirebaseManager:
    def __init__(self):
        ...
        # authomatically sign the player in anonymously
    ...
    def create_game(self, initial_board_state: dict) -> str:
        try:
            # note: Pyrebase requires user idToken for secure writes
            new_game = self.db.child('games').push({
                'board_state': initial_board_state,
                'status': 'waiting',
                'created_at': int(time.time() * 1000)
                }, self.user['idToken'])
            return new_game['name'] # Pyrebase uses 'name for generated ID
    ...
    def listen_for_moves(self, game_id: str, callback: Callable[[dict], None]) -> None:
        ...
        # Wrapper to simulate the old firebase_admin event object
        class DummyEvent:
        ...
        def _handle_event(message):
            if message.get('data'):  
                print(f"DEBUG: Received move data...")
    ...
    def cleanup(self):
        ...
        for name, listener in listeners:
            try:
                print(f"🔥 FIREBASE: Closing {name}")
                listener.close()
            except Exception as e:
                print(f"🔥 FIREBASE ERROR on {name}: {e}")
